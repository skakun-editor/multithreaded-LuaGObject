...
/* lightuserdata key to registry, containing table representing weak cache of known objects. */
static int cache;
...
/* Checks that given narg is object type and returns pointer to type instance representing it. */
static gpointer
...
/* Walks given type and tries to find the closest known match of the object present in the repo. If found, leaves found type table on the stack and returns real found gtype, otherwise returns G_TYPE_INVALID. */
static GType
...
/* Throws type error for object at given argument, gtype can optionally contain name of requested type. */
static int
...
/* This is workaround method for broken gi_object_info_get_*_function_pointer() in GI 1.32.0.(see https://bugzilla.gnome.org/show_bug.cgi?id=673282) */
gpointer
	...
	const gchar *(*getter)(GIObjectInfo *))
{
	...
	while (info != NULL) {
		...
		if (func_name &&
			gi_typelib_symbol(gi_base_info_get_typelib(GI_BASE_INFO(info)),
				func_name, &func)) {
...
object_refsink(lua_State *L, gpointer obj, gboolean no_sink)
{
	...
	/* Check whether object has registered fundamental 'ref' function. */
	GIObjectInfo *info = GI_OBJECT_INFO(
		gi_repository_find_by_gtype(lua_gobject_gi_get_repository(), gtype));
	if (info == NULL)
		info = GI_OBJECT_INFO(gi_repository_find_by_gtype(
			lua_gobject_gi_get_repository(), G_TYPE_FUNDAMENTAL(gtype)));
	if (info != NULL && gi_object_info_get_fundamental(info)) {
		GIObjectInfoRefFunction ref =
			lua_gobject_object_get_function_ptr(info,
	...
	/* There is no known easy to ref this kind of object. But this typically appears when handling GParamSpec, and GParamSpec handling generally works fine even without ref/unref, so the warnings produced are generally junk, so disabled until a way to handle ParamSpec properly is found. */
#if 0
...
object_unref(lua_State *L, gpointer obj)
{
	...
	custom unref method. */
	GIObjectInfo *info = GI_OBJECT_INFO(gi_repository_find_by_gtype(
		lua_gobject_gi_get_repository(), gtype));
	if (info == NULL)
		info = GI_OBJECT_INFO(gi_repository_find_by_gtype(
			lua_gobject_gi_get_repository(), G_TYPE_FUNDAMENTAL(gtype)));
	if (info != NULL && gi_object_info_get_fundamental(info)) {
	...
	gboolean nothrow, gboolean transfer)
{
	...
	if (!nothrow &&(!obj ||(gtype != G_TYPE_INVALID
...
lua_gobject_object_2lua(lua_State *L, gpointer obj, gboolean own, gboolean no_sink)
{
	...
	if (!lua_isnil(L, -1))
	{
		...
		/* If the object was already owned, remove one reference, because our proxy always keeps only one reference, which we already have. */
		if (own)
		object_unref(L, obj);
		return 1;
	}
	...
	*(gpointer *) lua_newuserdata(L, sizeof(obj)) = obj;
	...
	/* If we don't own the object, take its ownership(and also remove floating reference if there is any). */
	if (!own)
...
object_env(lua_State *L)
{
	...
	lua_rawget(L, -2);
	if (!lua_isnil(L, -1))
		/* Object's env table for the object is attached to the controlling userdata in the 'env' table. */
		lua_getfenv(L, -1);
	else {
		...
		/* Create userdata guard, which disconnects env when the state dies. Attach the actual env table as env table to the guard
		udata. */
...
object_new(lua_State *L)
{
	if (lua_islightuserdata(L, 1))
		...
		/* Normally Lua code uses GObject.Object.new(), which maps directly to g_object_newv(), but for some reason GOI < 1.0 does not export this method in the typelib. */
		...
		gparam_info = gi_repository_find_by_name(
			lua_gobject_gi_get_repository(), "GObject", "Parameter");
		*lua_gobject_guard_create(L,
			(GDestroyNotify) gi_base_info_unref) = gparam_info;
		...
		for (i = 0; i < size; ++i) {
			...
			lua_gobject_record_2c(L, -2, &params[i],
