...
typedef struct _Param {
	...
	/* Flag indicating whether this parameter is represented by Lua input and/or returned value.  Not represented are e.g. callback's user_data, array sizes etc. */
	guint internal : 1;

	/* Flag indicating that this is internal user_data value for the callback.  This parameter is supplied automatically, not explicitely from Lua. */
	guint internal_user_data : 1;

	/* Set to nonzero if this argument is user_data for closure which is marked as(scope call). */
	guint call_scoped_user_data : 1;

	/* Number of closures bound to this argument.	0 if this is not user_data for closure. */
	guint n_closures : 4;

	/* Type of the argument, one of ParamKind values. */
	guint kind : 2;

	/* Index into env table attached to the callable, contains repotype table for specified argument. */
	guint repotype_index : 4;
} Param;

/* Structure representing userdata allocated for any callable, i.e. function, method, signal, vtable, callback... */
...
typedef struct _FfiClosure {
	...
	union {
		struct {
			...
			/* Callable's target to be invoked(either function, userdata/table with __call metafunction or coroutine(which is resumed instead of called). */
			int target_ref;
		};

		/* Closure's entry point, stored only temporarily until closure is created. */
		gpointer call_addr;
	};

	/* Flag indicating whether closure should auto-destroy itself after it is called. */
	guint autodestroy : 1;
...
/* Structure containing closure block. This is user_data block for C-side closure arguments. */
struct _FfiClosureBlock {
	...
	/* Number of other closures in the block, excluding the forst one contained already in this header. */
	int closures_count;

	/* Variable-length array of pointers to other closures. Unfortunately libffi does not allow to allocate contiguous block containing more closures, otherwise this array would simply contain FfiClosure instances instead of pointers to dynamically allocated ones. */
	...
	if (ffi == NULL) {
		...
		if (tag == GI_TYPE_TAG_INTERFACE) {
			...
			if (GI_IS_ENUM_INFO(ii) || GI_IS_FLAGS_INFO(ii))
				ffi = get_simple_ffi_type(
					gi_enum_info_get_storage_type(GI_ENUM_INFO(ii)));
...
/* If typeinfo specifies array with length parameter, mark it in specified callable as an internal one. */
static void
...
callable_allocate(lua_State *L, int nargs, ffi_type ***ffi_args)
{
	...
	*ffi_args =(ffi_type **) &callable[1];
	...
	/* Clear all 'internal' flags inside callable parameters, parameters are then marked as internal during processing of their parents. */
...
lua_gobject_callable_create(lua_State *L, GICallableInfo *info, gpointer addr)
{
	...
	if (GI_IS_FUNCTION_INFO(info)) {
		...
		if (!gi_typelib_symbol(
				gi_base_info_get_typelib(GI_BASE_INFO(info)),
					symbol, &callable->address))
			/* Fail with the error message. */
			return luaL_error(L, "could not locate %s(%s): %s",
						lua_tostring(L, -3), symbol, g_module_error());
	}
	else if (GI_IS_SIGNAL_INFO(info))
		/* Signals always have 'self', i.e. the object on which they are emitted. */
		callable->has_self = 1;
	...
	*ffi_arg++ = &ffi_type_pointer;
	...
	for (argi = 0; argi < nargs; argi++, ffi_arg++) {
		...
		*ffi_arg =(param->dir == GI_DIRECTION_IN)
		...
		if (gi_arg_info_get_closure_index(&param->ai, &arg)) {
			...
			/* `arg` is defined also on callbacks, so check for invalid scope to avoid setting the internal flag on them. */
			if (data_param != NULL
					&& gi_arg_info_get_scope(&data_param->ai)
		...
		/* In case that we have an out or inout argument and callable returns boolean, mark it as ignore_retval(because we will signalize failure by returning nil instead of extra value). */
		if (param->dir != GI_DIRECTION_IN
			&& gi_type_info_get_tag(callable->retval.ti)
				== GI_TYPE_TAG_BOOLEAN)
			callable->ignore_retval = 1;
	}

	/* Manual adjustment of 'GObject.ClosureMarshal' type, which is crucial for lua_gobject but is missing an array annotation in glib/gobject-introspection < 1.30. */
	if (!GLIB_CHECK_VERSION(2, 30, 0)
			&& !strcmp(gi_base_info_get_namespace(GI_BASE_INFO(info)),
				"GObject")
			&& !strcmp(gi_base_info_get_name(GI_BASE_INFO(info)),
				"ClosureMarshal")) {
	...
	if (ffi_prep_cif (&callable->cif, FFI_DEFAULT_ABI,
		...
		lua_concat(L, lua_gobject_type_get_name(L,
...
/* Parses single 'Param' structure from the table on the top of the stack. Pops the table from the stack. */
static void
callable_param_parse(lua_State *L, Param *param)
{
	...
	if (kind == -1) {
		...
		/* Get type, assume record(if not overriden by real giinfo type below). */
		lua_getfield(L, -1, "type");
		if (!lua_isnil(L, -1)) {
			/* This is actually an enum, and 'type' field contains numeric type for this enum.  Store it into the ti. */
			GITypeInfo **ti = luaL_checkudata(L, -1, LUA_GOBJECT_GI_INFO);
			param->ti = GI_TYPE_INFO(gi_base_info_ref(*ti));
		}
		lua_pop(L, 1);

		/* Finally get the type from the table(from index 1) and replace the table with the type. */
		lua_rawgeti(L, -1, 1);
		lua_replace(L, -2);
	}
...
lua_gobject_callable_parse(lua_State *L, int info, gpointer addr)
{
	...
	/* Create ffi_cif. */
	if (ffi_prep_cif (&callable->cif, FFI_DEFAULT_ABI,
			nargs + callable->throws, ffi_retval, ffi_args) != FFI_OK)
...
{
	int nret = 0;
	if (param->kind == PARAM_KIND_ENUM
			&& lua_type(L, narg) != LUA_TNUMBER) {
...
callable_call(lua_State *L)
{
	...
	/* Prepare proper call->ffi_args[] pointing to real args(or redirects in case of inout/out parameters). Note that this loop cannot be merged with following marshalling loop, because during marshalling of closure or arrays marshalling code can read/write values ahead of currently marshalled value. */
	...
	for (i = 0; i < callable->nargs; i++, param++) {
		...
		if (param->n_closures > 0) {
			args[argi].v_pointer = lua_gobject_closure_allocate(L,
				param->n_closures);
			if (param->call_scoped_user_data)
				/* Add guard which releases closure block after the call. */
				*lua_gobject_guard_create(L, lua_gobject_closure_destroy)
					= args[argi].v_pointer;
		}
	}
	...
	for (i = 0; i < callable->nargs; i++, param++)
		if (!param->internal) {
			...
			nret += callable_param_2c(L, param, lua_argi++, 0, &args[argi],
					1, callable, ffi_args);
			/* Special handling for out/caller-alloc structures; we have to manually pre-create them and store them on the stack. */
			else if (callable->info && gi_arg_info_is_caller_allocates(&param->ai)
					&& lua_gobject_marshal_2c_caller_alloc(L, param->ti,
						&args[argi], 0)) {
				...
				/* Normal OUT parameters.  Ideally we don't have to touch them, but see https://github.com/lgi-devs/lgi/issues/118 */
	...
	if (!callable->ignore_retval
		...
		/* Make sure that returned boolean is converted according to ffi_call rules. */
		union {
			GIArgument arg;
			ffi_sarg s;
		} *ru =(gpointer) &retval;
	...
	for (i = 0; i < callable->nargs; i++, param++)
		if (!param->internal && param->dir != GI_DIRECTION_IN)
		{
			if (callable->info && gi_arg_info_is_caller_allocates(&param->ai)
					&& lua_gobject_marshal_2c_caller_alloc(L,
						param->ti, NULL, -caller_allocated  - nret))
				/* Caller allocated parameter is already marshalled and lying on the stack. */
				caller_allocated--;
			else {
			...
			/* In case that this callable is in ignore-retval mode and function actually returned FALSE, replace the already marshalled return value with NULL. */
	...
	/* When function can throw and we are not returning anything, be sure to return at least 'true', so that caller can check for error in a usual way(i.e. by Lua's assert() call). */
	...
	if (!param->internal && param->dir != GI_DIRECTION_OUT) {
		if G_LIKELY(i != 3 || !callable->is_closure_marshal) {
			...
			callable_param_2lua(L, param, real_arg, 0,
				callable_index, callable, args + callable->has_self);
		} else {
			/* Workaround incorrectly annotated but crucial ClosureMarshal callback.  Its 3rd argument is actually an array of GValue, not a single GValue as missing annotation suggests. */
...
marshal_return_values(lua_State *L, void *ret, void **args, int callable_index, Callable *callable, int npos)
{
	...
	if (tag != GI_TYPE_TAG_VOID
			|| gi_type_info_is_pointer(callable->retval.ti))	{
		if (callable->ignore_retval)
			/* Return value should be ignored on Lua side, so we have to synthesize the return value for C side.  We should return FALSE if next output argument is nil. */
			...
			if (to_pop != 0) {
				g_warning("cbk `%s.%s': return(transfer none) %d, unsafe!",
					...
					gi_base_info_get_name(
						GI_BASE_INFO(callable->info)), to_pop);
				lua_pop(L, to_pop);
			}
	...
	if (!param->internal && param->dir != GI_DIRECTION_IN) {
		...
		to_pop = callable_param_2c(L, param, npos,
			caller_alloc ? LUA_GOBJECT_PARENT_CALLER_ALLOC : 0,
			*arg, callable_index, callable, args + callable->has_self);
		if (to_pop != 0) {
...
marshal_return_error(lua_State *L, void *ret, void **args, Callable *callable)
{
	...
	if (*err == NULL) {
		...
		GQuark q = g_quark_from_static_string(
			"lua_gobject-callback-error-quark");
...
closure_callback(ffi_cif *cif, void *ret, void **args, void *closure_arg)
{
	...
	/* Remove callable userdata from callable_index, otherwise they mess up carefully prepared stack structure. */
	...
	/* Reintroduce callable to the stack, we might need it during marshalling of the response. Put it right before all returns. */
	lua_rawgeti(marshal_L, LUA_REGISTRYINDEX, closure->callable_ref);
	...
	/* If the closure is marked as autodestroy, destroy it now. Note that it is unfortunately not possible to destroy it directly here, because we would delete the code under our feet and crash and burn :-(. Instead, we create marshal guard and leave it to GC to destroy the closure later. */
	if (closure->autodestroy)
		*lua_gobject_guard_create(L, lua_gobject_closure_destroy) = block;

	/* This is NOT called by Lua, so we better leave the Lua stack we used pretty much tidied. */
...
/* Creates container block for allocated closures.  Returns address of the block, suitable as user_data parameter. */
gpointer
